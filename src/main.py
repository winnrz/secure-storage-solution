...
import os
import json
from key_generation.load_priv_keys import load_priv_keys
...
if __name__ == "__main__":
  ...
  e_pub_alice = generate_ed25519_keypair("alice", passphrase="alice_passphrase")

  # Create a X25519 key pair
  x_pub_alice = generate_x25519_keypair("alice", passphrase="alice_passphrase")

  # Create a Ed25519 key pair
  e_pub_bob =generate_ed25519_keypair("bob", passphrase="bob_passphrase")

  # Create a X25519 key pair
  x_pub_bob = generate_x25519_keypair("bob", passphrase="bob_passphrase")

  # Generate ephemeral key pair
  ephemeral_priv, ephemeral_pub = generate_ephemeral_x25519_keypair()

  # Load keys from disk
  e_priv_alice, x_priv_alice, x_priv_bob, e_priv_bob = load_priv_keys(passphrases={
      "e_priv_alice": "alice_passphrase",
      "x_priv_alice": "alice_passphrase",
      "e_priv_bob": "bob_passphrase",
      "x_priv_bob": "bob_passphrase"
  })
  ...
  ciphertext_bytes = aes_gcm_encrypt_file(aes_key, "plaintext/plaintext.txt", "encryption_res/ciphertext.enc", aad=aad)
  ...
  # Sign the package with sender's Ed25519 private key
  digital_signature = sign_message(e_priv_alice, package)

  #  Decode the package
  ephemeral_pub_bytes, salt_bytes, ciphertext_bytes = decode_package(package)

  # Verify the sender's signature
  is_valid = verify_signature(e_pub_alice, digital_signature, package)
  ...
  decrypted_path = aes_gcm_decrypt_file(aes_key, ciphertext_bytes, "decryption_res/plaintext_decrypted.txt", aad=aad)
